...
class MarkovChain {
    ...
    generate(length, startText = '', type = 'char') {
        ...
        if (startText && startText.trim().length > 0) {
            const startTokens = type === 'word' ? 
                startText.trim().split(/\s+/).filter(t => t.length > 0) :
                startText.split('');
    ...
    getStats() {
        return {
            ...
            avgTransitions: this.ngrams.size > 0 ? 
                Array.from(this.ngrams.values()).reduce((sum, transitions) => sum + transitions.length, 0) / this.ngrams.size : 0
        };
    }
    ...
    getSampleProbabilities(maxSamples = 5) {
        ...
        for (let i = 0; i < Math.min(maxSamples, ngrams.length); i++) {
            ...
            const probabilities = uniqueTransitions.map(token => {
